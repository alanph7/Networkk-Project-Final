...
export default function Search() {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        setFilteredProviders(response.data); // Set the fetched data
  ...
  const applyFilters = () => {
    ...
    let filtered = filteredProviders.filter((provider) => {
      return (
        provider.type.toLowerCase().includes(filters.type.toLowerCase()) &&
        (!filters.minPrice || provider.basePrice >= parseInt(filters.minPrice)) &&
        (!filters.maxPrice || provider.basePrice <= parseInt(filters.maxPrice)) &&
        (!filters.maxDistance ||
          provider.distance <= parseInt(filters.maxDistance)) &&
        (!filters.minRating ||
          provider.avgRating >= parseFloat(filters.minRating)) &&
        (!filters.name ||
          provider.serviceProvider.fname
            .toLowerCase()
    ...
    filtered.sort((a, b) => {
      if (filters.sortBy === "price") {
        ...
        return b.avgRating - a.avgRating; // Always sort ratings in descending order
      } else if (filters.sortBy === "distance") {
        return filters.sortOrder === "asc"
          ? a.distance - b.distance
          : b.distance - a.distance;
        ...
        {filteredProviders.map((provider) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <p className="font-bold text-lg">${provider.basePrice}/hr</p>
            </div>
          </div>
        ))}
      </div>
