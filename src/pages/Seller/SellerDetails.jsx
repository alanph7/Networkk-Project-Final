...
const SellerDetailsForm = () => {
  ...
  const handleAddSkill = () => {
    if (currentSkill && !skillsList.includes(currentSkill)) {
      const newSkills = [...skillsList, currentSkill];
  ...
  const handleAddLanguage = () => {
    if (currentLanguage && !languagesList.includes(currentLanguage)) {
      const newLanguages = [...languagesList, currentLanguage];
  ...
  const handleRemoveLanguage = (languageToRemove) => {
    const newLanguages = languagesList.filter(lang => lang !== languageToRemove);
    setLanguagesList(newLanguages);
    setFormData(prev => ({ ...prev, languages: newLanguages.join(', ') }));
  };
      ...
      {isEditing ? (
        ...
        <div className="flex flex-wrap gap-2">
          {languagesList.map((language, index) => (
            <span
              key={index}
              className="inline-flex items-center px-3 py-1 bg-gray-100 rounded-full"
            >
              {language}
            </span>
          ))}
